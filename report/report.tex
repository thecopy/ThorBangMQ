   ...
   \begin{figure}[H]
    ...
    \section{Analysis}
        ...
        \subsection{Stability of the system}
            In our long trace we saw that our system behaved in a stable manner over four hours. With this result we allowed ourselves to assume that some 10-minute tests which we performed with similar parameters will behave in a stable manner as well.\\
            \\
            When comparing the throughput and the response time of Figure \ref{table:2k2-threads-clients} for 750 clients and 100 worker threads with the results for 750 clients and 30 worker threads, we see that the throughput stays about the same and that the response time goes up. From this we can see that the increase in clients means that queues are building up in the middleware. Seeing that the confidence interval stays low for both 750 clients and 425 clients, we see that the system stays stable also for these configurations, which means that we aren't putting enough load on the system to make it unstable. We note that the confidence interval for 750 clients is much bigger than it is for 425 and 150 clients, which shows that the amount of load we can put on the system without making it unstable is around 425 clients running \textit{Standard test}.
