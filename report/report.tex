...
\begin{document}
    ...
    \section{Tests and results}
        ...
        \subsection{Long traces}
            ...
            \begin{figure}[H]
                \centering
                \centerline{\includegraphics[scale=0.75]{2h_100threads_standardTest_100_50}}
        ...
        \subsection{Difference in database connections and worker threads}
            ...
            The raw data from these tests can be found in the log-folder, in the subfolders
            \begin{itemize}
                \item \textit{sendandpopsameclient_diff_worker-threads_db-connections}
            \end{itemize}
        ...
        As discussed in Section \ref{sec:tests_and_results}, all our deployments live on a cloud hosting platform and thus are subject to neighbour noise. To demonstrate that the difference in computing power avaiable to our system varies over time an identical test were run on the evening (~20:00) on day, and once again in the morning the next day (~10:00). The difference in throughput and response time is presented in Figure \ref{fig:same_test_different_time_of_day} and \ref{fig:same_test_different_time_of_day_resp_time_over_time}.
