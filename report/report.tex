 ...
 7& 345.78 &    76.72 &  4247.42\\ \hline    
    ...
    \section{Analysis}
        ...
        \subsection{Number of middlewares}
		   ...
		   In Figure \ref{fig:throughput_middleware_clients} it is shown that when we increase the number of clients across one and two middleware, they hit the throughput threshold at the number of clients. We can also see that while the troughput is the same for the system the response time is lower
        ...
        \subsection{Bound on throughput of middleware}
            ...
            % TODO: PLOT DATA FOR THE FOLLOWING ARGUMENT
    ...
    \section{Conclusion}
        TODO: Say something about the database configuration and how we possibly should've chosen different indexes
...
% Increasing # middleware can give a lower response time, if there are enough requests (vs worker threads and db cons) to form queues in the middleware
        ...
        In Section \ref{sec:difference_in_dbcons_and_worker_threads} we found that the performance of our middleware depends on the ratio between the amount of worker threads and database connections it has. Here, we found that this ratio should be 1 to get the best performance while not having idle resources. This was exactly the result we expected to see.
% It's better to have more worker threads, but only up to a certain point. We have data that shows that it's better to have 30 worker threads than 100 in some situations
        TODO: Say something about what we could've done differently, for instance tested with different indexes on the database
    \clearpage
    \appendix
