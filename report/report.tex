...
\begin{document}
    ...
    \section{Tests and results}
        ...
        \subsection{Long traces}
            The figures in this section show the overall stability of the system while under moderate to high load for 2 hours or more. Each of the following tests are of the type \textit{Standard test} and have been run with different parameters, increasing the number of database connections and worker threads, and the number of clients. In these tests we expect to see that the system behaves in a somewhat stable manner (at least for lower numbers of clients) meaning that we have a low standard deviation of our measurements. We expect that the throughput will go up as the number of clients and worker-threads and database connections increases since, as described in Section \ref{sec:description_middleware}, the minimum of the number of database connections and worker-threads bounds the amount of requests that will be handled simultaneously by our middleware. This shouldn't scale infinitely though, since we put more load on both the middleware and the database by increasing these factors.\\
            \\
            ...
            \begin{figure}[H]
                \centering
                \centerline{\includegraphics[scale=0.70]{2h_100threads_standardTest_100_50}}
                \caption{On the left we have plotted the throughput of the system while under load from \textit{Standard Test}, while on the right we have plotted the response time. The test was performed with 100 one-way clients and 50 two-way clients, using 1 middleware with 100 worker threads and 100 database connections. The dotted red line on the throughput graph shows the average throughput and the solid lines the standard deviation.}
                \label{fig:2h_100threads_standardTest_100_50}
            \end{figure}
            ~\\
            \\
            On Figure \ref{fig:2h_100threads_standardTest_100_50}, where we have a total of 150 clients, using 100 worker threads and database connections in the middleware, we see that the throughput of both \textit{Pop message} and \textit{Send message} stays very stable throughout the whole test, showing an average of (TODO) \textit{Pop message} and (TODO) \textit{Send message} per second, with a standard deviation of (TODO) and (TODO) respectively.\\
            \\
            ...
            \begin{figure}[H]
                \hspace{-1.5cm}
                \centerline{\includegraphics[scale=0.700]{2h_30threads_standardTest_500_250}}
            ...
            \begin{figure}[H]
                \centering
                \centerline{\includegraphics[scale=0.700]{2h_100threads_standardTest_500_250}}
        ...
        \subsection{Micro benchmarks}
            ...
            \subsubsection{Peek message}
                ...
                \begin{figure}[H]
                    ...
                    \caption{The response time for \textit{Send message} request. Each data point represents an individual request. A total of 500,000 requests sent sequentially each after the other.}
        ...
        \subsubsection{1 vs 10 middlewares}    
            In this test we we ran two \textit{Standard tests} with 1000 one-way clients and 500 two-way clients, measuring the difference in performance when going form 1 middleware to 10 middlewares.
        ...
        \subsubsection{Response Times for Requests}
    		...
    		\begin{table}[H]
    			...
    			\hline 
    			\end{tabular} 
                \caption{$2^2$ testing for throughput, varying number of client and number of threads. Test used: \textit{Standard test}}
    ...
    \section{Analysis}
        ...
        \subsection{Stability of the system}
            ...
            \\
            When comparing the throughput and the response time of Figure \ref{fig:2h_30threads_standardTest_500_250} (750 clients, 100 worker threads) and \ref{fig:2h_100threads_standardTest_500_250} (750 clients, 30 worker threads), we see that the throughput stays about the same and that the response time goes up. From this we can see that the increase in clients means that queues are building up in the middleware. Even when we have queues in the middleware, we see that the system behaves in a stable manner.\\
            \\
            When we compare the throughput of Figure \ref{fig:2h_100threads_standardTest_100_50} and \ref{fig:2h_100threads_standardTest_500_250} we see that the throughput increases when the number of clients increases. We see that the standard deviation (see numbers in Table \ref{table:long_trace_test_summary_sendmessage} and \ref{table:long_trace_test_summary_popmessage}) increases 5 fold. This increase in standard variation shows that we are pushing the system a lot more with 750 clients than we do with 150 clients. The low standard deviation we have with 150 clients shows that we aren't pushing the system to the point of making it unstable, but that we are on our way to this point when we use 750 clients.\\
            \\
